...
| `--session-status` | boolean |  |  |  | Ask the session what it is doing -- adapter, connection, uptime, requests, whether it is busy -- as JSON, and exit. Answered while a query is running. Needs --session. |

...
- Adds `hsql --spec`, which dumps hsql's own options and every installed adapter's connection options as JSON, so an agent can read the CLI surface instead of parsing `--help` ([#524](https://github.com/tconbeer/harlequin/issues/524)). Every option reports each way to spell it, what it takes, the values a choice accepts, its default, and whether it is repeatable — and an adapter option reports both spellings it has, `--read-only` for the command line and `read_only` for a profile. `-a NAME` narrows it to one adapter. It connects to nothing and reads no config file, so it answers even when your database is unreachable or your config is broken. The `harlequin` command's own options are not covered, and the document says so instead of reading as an exhaustive list.
- Adds `AbstractOption.to_dict()` to the adapter API: an option as plain data, for anything that reads options instead of rendering them. It is concrete rather than abstract, so an option class written before it existed keeps working.

...
- Adds `hsql --find TERM`, which searches the whole catalog for tables, views and columns whose name contains TERM instead of listing a level at a time: `hsql --find customer_id` says which tables have that column, and `--path` narrows the search. It works with the DuckDB and SQLite adapters; `hsql --info` reports which of your adapters can search, and one that cannot says so instead of walking its catalog.
...
- Adds an optional `HarlequinConnection.search_catalog()`, which returns every catalog item whose label contains a term, paired with the labels of its ancestors. Adapters that implement it should set `IMPLEMENTS_CATALOG_SEARCH = True`, which is what `hsql --find` and `hsql --info` read.

...
- `hsql --session NAME --session-status` reports what a session is doing as JSON — adapter, connection, uptime, requests, busy or idle — and answers even while a query is running.
- In Harlequin, press `alt+e` in the Query Editor to edit the current buffer in the editor named by `$VISUAL` or `$EDITOR`; quitting the editor with a non-zero status (like `:cq`) discards the changes ([#1102](https://github.com/tconbeer/harlequin/issues/1102)).
...
- When Harlequin raised an unhandled exception, it now saves your buffers and then exits with a short message instead of a traceback, writing a crash report you can attach to an issue ([#687](https://github.com/tconbeer/harlequin/issues/687)).
- `hsql` now exits `70` and writes a crash report when it encounters an unhandled exception, instead of printing a traceback and exiting `1` ([#687](https://github.com/tconbeer/harlequin/issues/687)).
- Harlequin and hsql now record every query in one shared history. Pass `--no-write-history` (or set `no_write_history = true` in a profile) to opt out ([#429](https://github.com/tconbeer/harlequin/issues/429)).

...
- A session now stops itself once it has gone 30 minutes with no request, or eight hours after it connected: `--idle-timeout SECONDS` and `--max-lifetime SECONDS` on `hsql --serve` set both, and `0` switches either off. `--session-status` reports them as `idle_timeout_s` and `expires_in_s`.
- A session that is left in a transaction mode it did not connect in now says so on stderr after every request.
- `hsql --serve` now warns when a password is typed on its command line, since a session's command line is readable in `ps` for as long as it runs. Start it from a profile (`-P`) instead.
...
- Harlequin and hsql now record every query as it runs in one shared history, so a crashed session keeps its history. Pass `--no-write-history` (or set `no_write_history = true` in a profile) to opt out ([#429](https://github.com/tconbeer/harlequin/issues/429)).

...
- A session that is left in a transaction mode it did not connect in now says so on stderr after every request, so an open transaction holding locks is visible rather than something the next caller finds out about.

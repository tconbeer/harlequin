...
def build_cli(argv: Sequence[str]) -> click.Command:
    ...
    def inner_cli(
        ...
        **kwargs: Any,
        ...
        run = _Run(deadline=deadline, served=served, log=query_log)
...
def _execute_all(
    ...
    *,
    ...
    if run.stopped:
        ...
        # so the one thing that stops it is not submitting one -- and so there
        # is no statement to record
        return executed
...
def _fetched(
    ...
    *,
    ...
    for position, item in enumerate(selected, start=1):
        if run.stopped:
            # the clock ran out, or the caller did, between statements, and
            # nothing after this one will be fetched either

...
`layout.py` does padding, pipes and row counts over the strings `text_columns()` produced, and knows nothing about types. A value holding a newline is laid out as psql lays one out — the cell spans physical lines, `+` in the separator marks each one that continues, and the columns beside it go blank — so a query's text survives a listing intact rather than being flattened into one that no longer runs. Widths are **terminal cells, via `wcwidth`** — never `len()`, which is off by one per CJK glyph or emoji and by one the other way per combining mark. Its `LayoutOptions` are independent switches on purpose: `-t` is `header=False, footer=False` and `-A` is `aligned=False`, so `-tA` needs no special case.
...
- `cli.py` parses in **two phases**, shared with the IDE in `harlequin.first_pass`. The first reads `-a`, `-P` and `--config-path` with a throwaway resilient parser to name the one adapter the invocation will use — `adapter_names()`, no `ep.load()` — and the second builds the real command with only that adapter's options. `--help` with no adapter named imports *nothing* and lists adapter names instead; that's what keeps the first thing an agent reads small and true for every adapter. `harlequin.query` and `harlequin.statements` are deferred into the callback for the same reason.
- **hsql's own flags are the frozen part.** An adapter option whose spelling collides with one loses that spelling (`first_pass.attach_adapter_options`, under the spellings hsql reserves), rather than shadowing a documented flag.
...
- `timeout.py` is `--timeout`: the work moves to a worker thread and the clock stays on the main one, and on expiry it cancels, waits a grace period, and — because an interpreter shutting down around a thread still inside a driver exits 134 — ends a run that will not stop with `os._exit()`. **The cancellation is hsql's to attribute**, because a cancelled cursor comes back empty and error-free, exactly like a query that matched nothing: the run reads the deadline between results and stops rather than printing that empty result. A run with no `--timeout` starts no thread. A session's cancel is the same shape one level up — the server marks the request, calls `connection.cancel()`, and the run reads that flag between results — and `IMPLEMENTS_CANCEL` is what decides whether a caller is stopping the query or only detaching from it.
- **Warm sessions** (`session.py`, `client.py`, `protocol.py`, `server.py`) are `hsql --serve NAME` holding a connection open and `hsql --session NAME` (or `HSQL_SESSION`) sending it invocations over a unix socket. What crosses the wire is stdout bytes, stderr bytes and an exit code, because the server runs the same command the cold path builds — which is what `tests/unit_tests/test_hsql_equivalence.py` pins. **`main()` decides which of the two an invocation is before it imports anything that is not stdlib**, and everything the client touches has to stay stdlib-only: the whole round trip is about a millisecond, and `import click` costs more. Requests run one at a time on one connection; `--session-status` and a `Ctrl-C`'s cancel arrive as their own frames and are answered off the server's bookkeeping, so both land while a query is running.
...
`secret=True` on an option is how an adapter says a value must never be printed back. Core cannot enumerate what is sensitive across every adapter, so the declaration is the mechanism and `harlequin.redact` is the only place that acts on it: `redact_profile()` masks a profile's values, `redact_conn_str()` masks the credentials inside a DSN (which no option describes, because `CONN_STR` is positional), and `redact_text()` is the backstop for a driver exception that quotes a DSN back. `hsql --info`, `--config show`, `--spec`, the generated schema and the IDE's debug screen all route through it, and `hsql`'s stderr does too — `redact.hide_secrets_in()` accumulates the values this process must not print — `redact_text()` applies them by default — and `diagnostics._write()` is the one line every message leaves through. `redact_profile()` also treats a key *named* like a password as one, as a backstop under the declaration rather than instead of it: every adapter's options predate `secret=`, and an adapter that is not installed on the machine reading the config declares nothing at all. `hsql --config show` pays an import per adapter its profiles name to get the declaration — the one mode that reports on config files and still reaches for an adapter — and says on stderr when it could not, because that is the one path where a value it would have masked might print.
...
`config_schema.py` is the third consumer of those same two passes, and generates rather than declares: `msgspec.json.schema()` over `Config` for the top level and over `adapter_options_model()` for each adapter's keys — the same struct `parse_profile_options()` parses into, so the schema's types are the ones a run enforces — plus click parameters for the keys a command reads and `AbstractOption.to_dict()` for each option's description and default. `hsql --config schema` writes it for the adapters installed on the machine it runs on — so `adapter` is an enum of what is there and an undeclared key under a profile is an error — and `scripts/write_config_schema.py` writes the same document for no adapters at all into `schemas/config-v1.json`, which is what harlequin.sh publishes. The file is a build artifact: nothing reads it at run time, and `tests/unit_tests/test_config_schema.py` is what fails when it and the generator disagree.

Writes go the other way, through `ConfigFile`'s tomlkit merge, and have two callers: `harlequin --config`, which prompts, and `hsql --config init`, which does not — `hsql` may never import questionary, so the wizard stays where it is and each command gets the affordance right for its audience. `init` writes the one profile `-P` names, from the options a caller actually typed, and hands `update()` the file's whole section back so the profiles and comments it was not asked to touch survive. It is the only mode that needs an adapter and no profile: the options it writes are the adapter's, and the profile it names is one that does not exist yet — which is why `first_pass()` asks those two questions separately.
...
- **Variable names are descriptive and explicit.** Name what a value *is*, not the fact that you have one: `first_pass_config`, not `found`; `installed_adapter_names`, not `installed`. A reader landing in the middle of a function should not have to scroll up to learn what a name holds. A single-word name is fine when the scope is a line or two and the loop says everything (`for option in adapter_cls.ADAPTER_OPTIONS`); anything read more than once, and anything crossing a function boundary, gets a name that stands on its own.

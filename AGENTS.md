...
Reading in priority order is what lets `get_profile()` stop at the file that defines the profile it was asked for; `load_config()` is the whole document, and reads everything. Resolving a name and reading the document are separate: a `default_profile` that names no profile is raised by `_select_profile()`/`get_profile()`, where the name is actually used, so `-P other` and `-P None` are not refused over a key they overrode — which is also what keeps the two commands agreeing wherever they read the same files. They still diverge where they do not: `hsql` cannot report a problem in a file it stopped before opening, and `--config validate` is the mode that will read everything. Validation is two passes and both are in `config.py`: `_validate_file()` checks what core owns *before* the merge, so an error can name the file it came from, and `validate_profile_options()` runs once the adapter is known and checks the profile's remaining keys against that adapter's declared options — the only place in the stack that can tell `reed_only` from `read_only`, since an adapter's constructor takes supersets of what it declares and drops the rest. Both are declared as msgspec models, built inside the functions that use them: msgspec costs ~30ms to import, and an invocation with no config file to validate must not pay it.
